    ...
    <div className="bg-white min-h-screen overflow-hidden">
      ...
      <section className="relative pt-24 pb-32 lg:pt-40 lg:pb-48 bg-gray-900 overflow-hidden">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10 text-center">
          ...
          <h1 className="text-5xl lg:text-7xl font-black text-white mb-8 leading-tight tracking-tight">
            Nurturing <span className="text-indigo-400">Global Thinkers</span> <br />
            in Every Classroom.
          </h1>
          <p className="text-xl text-gray-400 max-w-3xl mx-auto leading-relaxed mb-12">
            Kangaroo Pakistan (Innovative Learning) is the sole representative of Kangourou Sans Frontières, dedicated to bringing world-class educational opportunities to the youth of Pakistan.
          </p>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8 max-w-4xl mx-auto pt-8 border-t border-white/10">
            {stats.map((stat, i) => (
              <div key={i} className="text-center">
                ...
                <div className="text-indigo-400 text-xs font-bold uppercase tracking-widest">{stat.label}</div>
              </div>
            ))}
          </div>
        </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
            {pillars.map((pillar) => (
              ...
              >
                ...
                <h3 className="text-2xl font-black text-gray-900 mb-2">{pillar.title}</h3>
                <p className={`font-bold text-sm ${pillar.accent} mb-6 uppercase tracking-wider`}>{pillar.tagline}</p>
                <p className="text-gray-600 mb-8 text-sm leading-relaxed">
                  {pillar.description}
                </p>
                <ul className="space-y-3">
                  {pillar.points.map((point, pi) => (
                    <li key={pi} className="flex items-start gap-3 text-xs font-semibold text-gray-500">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex flex-col lg:flex-row gap-20 items-center">
            <div className="lg:w-1/2">
              <div className="inline-block px-4 py-1.5 bg-indigo-100 text-indigo-700 rounded-full text-xs font-bold mb-6">OUR STORY</div>
              <h2 className="text-4xl lg:text-5xl font-black text-gray-900 mb-8 leading-tight">
                From a Small Idea <br />to a <span className="text-indigo-600">Global Movement.</span>
              </h2>
              <div className="space-y-6 text-gray-600 leading-relaxed">
                <p>
                  In the early 80's, Peter O'Holloran invented a new kind of game in Australian schools that allowed thousands of students to participate simultaneously. This success inspired French educators André Deledicq and Jean Pierre Boudine to launch "Kangaroo" in 1991 in France.
                </p>
                <p>
                  Today, <strong>Kangourou Sans Frontières (KSF)</strong> is a global powerhouse of educational innovation. Pakistan joined this initiative in 2005, and since then, we have empowered millions of students across the country to demonstrate that their abilities are unmatched at the international level.
                </p>
              </div>
              <div className="mt-10 flex gap-6">
                <div className="flex flex-col">
                  <span className="text-3xl font-black text-indigo-600">6M+</span>
                  <span className="text-xs font-bold text-gray-400 uppercase">Annual Participants</span>
                </div>
                <div className="w-px h-12 bg-gray-200"></div>
                <div className="flex flex-col">
                  <span className="text-3xl font-black text-indigo-600">120+</span>
                  <span className="text-xs font-bold text-gray-400 uppercase">Member Countries</span>
                </div>
              </div>
            </div>
            <div className="lg:w-1/2 relative">
              <div className="aspect-[4/5] bg-white rounded-[60px] p-4 shadow-2xl relative z-10 overflow-hidden">
                 <img src="https://picsum.photos/seed/kangaroo-history/800/1000" alt="Contest history" className="w-full h-full object-cover rounded-[50px] grayscale hover:grayscale-0 transition-all duration-700" />
              </div>
              {/* Decorative blob */}
      ...
      <section className="py-32">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          <h2 className="text-4xl font-black text-gray-900 mb-4">Our Visionary Team</h2>
          <p className="text-gray-500 max-w-2xl mx-auto mb-20 text-lg">
            Meet the dedicated professionals working tirelessly to ensure Pakistani students stay connected with global educational standards.
          </p>
            ...
            {TEAM.map((member) => (
              <div key={member.name} className="group">
                <div className="mb-6 relative mx-auto w-full aspect-square max-w-[200px] rounded-[40px] overflow-hidden shadow-xl group-hover:shadow-indigo-200 group-hover:-translate-y-2 transition-all duration-500">
                  <img src={member.image} alt={member.name} className="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all duration-700 scale-110 group-hover:scale-100" />
                ...
                <p className="text-indigo-600 font-bold text-xs uppercase tracking-widest">{member.role}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
          <div className="flex flex-col lg:flex-row items-center justify-between gap-16">
            <div className="lg:w-1/3 text-center lg:text-left">
              <h2 className="text-4xl lg:text-5xl font-black mb-6">Global <br />Credentials.</h2>
              <p className="text-indigo-100/70 mb-10 leading-relaxed">
                We are proud to hold official memberships with international educational associations, ensuring our processes meet global benchmarks.
              </p>
              <div className="flex flex-col gap-4">
                <div className="p-4 bg-white/10 rounded-2xl border border-white/10 backdrop-blur-sm">
                   <p className="text-xs font-bold uppercase tracking-widest text-indigo-300">AFFILIATION 01</p>
                   <p className="font-bold">Kangourou Sans Frontières (KSF)</p>
                </div>
                <div className="p-4 bg-white/10 rounded-2xl border border-white/10 backdrop-blur-sm">
                   <p className="text-xs font-bold uppercase tracking-widest text-indigo-300">AFFILIATION 02</p>
                   <p className="font-bold">International Linguistic Association</p>
                </div>
              </div>
            </div>
      ...
      {activeCert && (
        ...
        >
          <div className="max-w-4xl w-full h-full bg-white rounded-[50px] overflow-hidden relative shadow-2xl flex flex-col" onClick={e => e.stopPropagation()}>
            <button
              onClick={() => setActiveCert(null)}
