...
const Navbar: React.FC = () => {
  ...
  return (
    <nav className="sticky top-0 z-50 bg-white/90 backdrop-blur-xl border-b border-gray-100 shadow-sm" ref={dropdownRef}>
          ...
          <div className="hidden lg:flex items-center space-x-1">
            {navLinks.map((link) => (
              <Link
                ...
                className={`px-4 py-2 rounded-lg text-gray-600 hover:text-indigo-600 font-semibold transition-all ${
                  location.pathname === link.path ? 'text-indigo-600 bg-indigo-50/50' : ''
                }`}
              >
                {link.name}
              </Link>
            ...
            {contests.map((type) => (
              <div key={type} className="relative group">
                ...
                >
                  {type}
                  <svg className={`w-4 h-4 transition-transform duration-200 ${activeDropdown === type ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  ...
                  >
                    <div className="px-4 py-2 mb-2 border-b border-gray-50">
                      <div className="flex items-center gap-2">
                        <img src={CONTESTS[type].logoUrl} alt="" className="w-6 h-6 object-contain" />
                        <span className="text-xs font-black text-gray-400 uppercase tracking-widest">{CONTESTS[type].fullName}</span>
                      </div>
                    </div>
                    {subLinks.map((sub) => (
                      <Link
            ...
            <Link
              to="/contact"
              className={`px-4 py-2 rounded-lg text-gray-600 hover:text-indigo-600 font-semibold transition-all ${
                location.pathname === '/contact' ? 'text-indigo-600 bg-indigo-50/50' : ''
              }`}
            >
              Contact
            </Link>
        ...
        <div className="lg:hidden bg-white border-t border-gray-100 max-h-[calc(100vh-80px)] overflow-y-auto py-4 px-4 space-y-1 animate-in slide-in-from-top duration-300">
          ...
          {contests.map((type) => (
            <div key={type} className="space-y-1">
              ...
              >
                {type}
                <svg className={`w-5 h-5 transition-transform ${activeDropdown === type ? 'rotate-180' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 9l-7 7-7-7" />
                </svg>
              </button>
              {activeDropdown === type && (
